...
- **Navigation**: Comprehensive menu system with "Shop by Brand" (6 brands: ENGWE, Eleglide, DYU, Duotts, Touroll, Fiido) and "Shop by Type" (3 categories: City, Folding, Mountain) dropdowns on desktop and mobile.
...
- **Admin Variant Management**: Products with variants (colours, wheel sizes, battery options) automatically expand in the admin inventory view. Each product displays a variant count badge, and the variant management interface provides clear controls to adjust price and stock for each variant individually with visual before/after indicators.
